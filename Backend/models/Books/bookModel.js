...
class Book {
  ...
  async getTitle(acc_no, token) {
    try {
      const { data } = await axios.get(
        `http://localhost:2150/books/getTitle/${acc_no}`,
        {
          headers: {
      ...
      console.log("Books Data Title n available copies", data);
      if (data.success) {
        return {
          success: true,
          data: data.data,
  ...
  async getTotalBooks() {
    try {
      const all_issued_books = await axios.get(
        "http://localhost:2150/books/getAllIssuedCount",
      );
      const issued_books = await axios.get(
        "http://localhost:2150/books/getTotalIssuedBookCount",
      );
      const books_count = await axios.get(
        "http://localhost:2150/books/getTotalBookCount",
      );
      const stu_count = await axios.get(
        "http://localhost:2150/student/getTotalStudents",
      );
      const online_stu = await axios.get(
        "http://localhost:2150/student/getOnlineStudents",
      );
      const overdue_books = await axios.get(
        "http://localhost:2150/books/getOverdueBooksCount",
      );
      ...
      if (
        all_issued_books.data.success &&
        issued_books.data.success &&
        books_count.data.success
      ) {
        return {
          success: true,
          data: [
  ...
  async AddBook(data, token) {
    try {
      const response = await axios.post(
        "http://localhost:2150/books/addBook",
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async seeTotalBooks() {
    try {
      const { data } = await axios.get(
        "http://localhost:2150/books/getAllBooks",
      );
  ...
  async searchBooks(term) {
    try {
      const { data } = await axios.get(
        `http://localhost:2150/books/searchBooks?term=${term}`,
      );
  ...
  async updateBook(book, token) {
    try {
      ...
      const { data } = await axios.put(
        "http://localhost:2150/books/updateBook",
        book,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async issueBook(issue_book, token) {
    try {
      const { data } = await axios.post(
        "http://localhost:2150/books/issueBook",
        issue_book,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.log("Result in the issueBook", data);
      if (data.success) {
        return {
          success: data.success,
          message: data.message,
      ...
      console.log("Error In The issueBook Model ", error);
      return {
        success: false,
        message: error.response.data.message || "Falied To Issue The Book",
      };
    }
  }
  //API Creation IS DONE
  ...
  async getIssuedBooks(token) {
    try {
      const result = await axios.get(
        "http://localhost:2150/books/getAllIssuedBooks",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async isBookExists(acc_no) {
    try {
      const result = await axios.get(
        `http://localhost:2150/books/isBookExists/${acc_no}`,
      );
  ...
  async fileReturnBook(data, token) {
    try {
      ...
      const result = await axios.put(
        "http://localhost:2150/books/fileReturn",
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      return {
        success: false,
        message: result.data.message || "Failed to return book",
      };
    } catch (error) {
      console.log("Error In the fileReturnBook Model", error);
      return {
        success: false,
        message: error.response?.data?.message || "Failed To Return The Book",
      };
    }
  }
}
